...
import { Tooltip, Button, message, notification } from 'antd';
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      notification.success({
        ...
        duration: 3,
        description: <>
          {`Username: ${user_info.data.user.username}`} <br/>
          {`Email: ${user_info.data.user.email}`} <br/>
          {`Intro: ${user_info.data.user.intro}`} <br/>
          {`Phone: ${user_info.data.user.phone}`}
        </>
      })
  
      if(response.status === 200) message.success('Đăng nhập thành công') 
    } catch (err) {
      console.log(err)
      message.error(err)
    }
        ...
        <div className="inputGroup inputGroup1">
          <label for="loginEmail" id="loginEmailLabel">
            ...
            <Tooltip
              ...
              title={
                <span>
                  Định dạng username hợp lệ: <br/>
                  - từ 5-20 ký tự.<br/>
                  - không bắt đầu và kết thúc bằng '_' hoặc '.'<br/>
                  - chỉ gồm các ký tự a-z, A-Z, 0-9, '_', '.'<br/>
                </span>
              }
          ...
          />
          <p className="helper helper1">user_123</p>
        </div>
